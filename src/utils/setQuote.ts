 ...
 * @LastEditors: cg
 * @LastEditTime: 2025-04-07 19:11:08
 */
...
export const setQuote = (editor: Editor, attributes: any) => {
  if (editor) {
    const { state } = editor
    ...
    state.doc.nodesBetween(from, to, (node, pos) => {
      if (node.isBlock || (node.isInline && !node.isText)) {
        if (node.type.name === NodeTypeEum.Blockquote) {
    ...
    setNodeOnOrderList(
      editor,
      editor.schema.nodes.IBlockquote,
      attributes,
      orderList,
      otherList,
      levelConfig
    ...
    //   convertNode(editor, item.pos, editor.schema.nodes.IBlockquote, { level })
    // })
  }
}
