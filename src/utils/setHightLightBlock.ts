 ...
 * @LastEditors: cg
 * @LastEditTime: 2025-04-08 16:50:50
 */
...
export const setHightLightBlock = (editor: Editor, attributes: any) => {
  if (editor) {
    const { state } = editor
    ...
    state.doc.nodesBetween(from, to, (node, pos) => {
      if (node.isBlock || (node.isInline && !node.isText)) {
        if (node.type.name === NodeTypeEum.HighLightBlock) {
    ...
    setNodeOnOrderList(
      editor,
      editor.schema.nodes.IHighLightBlock,
      attributes,
      orderList,
      otherList,
      levelConfig
