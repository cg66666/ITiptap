 ...
 * @LastEditors: cg
 * @LastEditTime: 2025-04-07 16:51:29
 */
...
export const setBulletList = (editor: Editor, attributes: any) => {
  if (editor) {
    const { state } = editor
    ...
    state.doc.nodesBetween(from, to, (node, pos) => {
      if (node.isBlock || (node.isInline && !node.isText)) {
        if (node.type.name === NodeTypeEum.BulleList) {
          bulletList.push({ node, pos })
        } else {
    ...
    setNodeOnOrderList(
      editor,
      editor.schema.nodes.IBulletList,
      attributes,
      // { tier: 1 },
      orderList,
      otherList,
    ...
    //   convertNode(editor, item.pos, editor.schema.nodes.IBulletList, { tier: 1 })
    // })
  }
}
